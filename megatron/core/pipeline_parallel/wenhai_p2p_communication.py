...
def _communicate(*, tensor_send_next: Optional[torch.Tensor],
    ...
    if use_ring_exchange_p2p:
        ...
        if tensor_recv_prev is not None:
            ...
            ops.append(recv_prev_op)
        if tensor_send_next is not None:
            # print(f"[communication] send next op to {get_pipeline_model_parallel_next_rank()}")
            send_next_op = torch.distributed.P2POp(
...
def wenhai_recv_forward(tensor_shape: Shape,
                 dtype: torch.dtype,
                 timers: Callable = None, is_first_day: bool = True) -> torch.Tensor:
    ...
    """
    # [Modified for wenhai]
    # if is not first day, need to recv from last pipeline stage of previous day
    # input tensor is ocean data predicted by model
    if core.parallel_state.is_pipeline_first_stage() and is_first_day:
        input_tensor = None 
    else:
        # print(f"[wenhai recv forward]<start> {core.parallel_state.get_pipeline_model_parallel_rank()}")
        ...
        input_tensor, _ = _communicate(
            ...
            dtype=dtype)
        # print(f"[wenhai recv forward]<end> {core.parallel_state.get_pipeline_model_parallel_rank()}")
        if timers is not None:
...
def wenhai_recv_backward(tensor_shape: Shape,
                  dtype: torch.dtype,
                  timers: Callable = None, is_last_day: bool = True) -> torch.Tensor:
    """Receive tensor from next rank in pipeline (backward receive).

    See _communicate for argument details.
    """
    if core.parallel_state.is_pipeline_last_stage() and is_last_day:
        output_tensor_grad = None
    else:
        if timers is not None:
            timers('backward-recv', log_level=2).start()
...
def wenhai_send_forward(output_tensor: torch.Tensor,
                 timers: Callable = None, is_last_day: bool = True) -> None:
    """Send tensor to next rank in pipeline (forward send).
    ...
    if not core.parallel_state.is_pipeline_last_stage():
        # print(f"[wenhai send forward]<start> {core.parallel_state.get_pipeline_model_parallel_rank()}")
        ...
        if timers is not None:
            timers('forward-send').stop()
        # print(f"[wenhai send forward]<end> {core.parallel_state.get_pipeline_model_parallel_rank()}")
...
def wenhai_send_backward(input_tensor_grad: torch.Tensor,
                  timers: Callable = None, is_first_day: bool = True) -> None:
    """Send tensor to previous rank in pipeline (backward send).

    See _communicate for argument details.
    """
    if not core.parallel_state.is_pipeline_first_stage() or not is_first_day:
        if timers is not None:
            timers('backward-send', log_level=2).start()
        _communicate(
            tensor_send_next=None,
