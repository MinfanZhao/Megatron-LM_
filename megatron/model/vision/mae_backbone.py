...
class MaskedAutoencoderViT (MegatronModule) :
    ...
    def __init__ (self,
        ...
        if args.init_method_xavier_uniform:
            self.init_method = torch.nn.init.xavier_uniform
        ...
        if self.add_decoder:
            ...
            if self.post_process:
                self.decoder_pred = torch.nn.Linear(self.decoder_embed_dim, self.num_patches * self.in_chans, bias=True)
        else:
            self.decoder = None
    ...
    def set_input_tensor(self, input_tensor):
        ...
        if self.add_encoder and self.pre_process:
            ...
            raise Exception('stage should have at least one of encoder or decoder')
    ...
    def forward(self, input):
        if self.pre_process and self.add_encoder:
            ...
            if self.class_token:
                ...
                cls_tokens = self.cls_token.expand(masked_output.shape[0], -1, -1) # b 1 h
                concatenated_tokens = torch.cat([cls_tokens, masked_output], dim=1) # b s+1 h
        ...
        if self.add_decoder:
            if self.pre_process:
                hidden_states = self.encoder_output
