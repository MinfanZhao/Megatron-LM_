...
def backward_step(optimizer, input_tensor, output_tensor,
    ...
    if input_tensor is not None:
        ...
        for j, x in enumerate (input_tensor):
            if x is None:
                input_tensor_grad.append(None)
            else:
                if x.requires_grad and j==0: # for mae
                    input_tensor_grad.append(x.grad)
                else:
                    input_tensor_grad.append(x) # for mae
    ...
    if mpu.get_pipeline_model_parallel_world_size() > 1 and \
            ...
            args.model_type == ModelType.encoder_and_decoder and args.vision_backbone_type != 'mae':
...
def get_tensor_shapes(rank, model_type):
    ...
    if args.vision_backbone_type == 'mae':
        # todo: add an argument :forward/backward
        # if forward: normal pass way
        # if backward: just append the tensor need to compute grad
        # no sequence_parallel 
        # seq_length = num_patches * (1-mask_ratio)+ cls_token(1)

        num_patches = (args.img_h // args.patch_dim) * (args.img_w // args.patch_dim)
        seq_length = int (num_patches * (1 - args.mask_ratio) + 1)
        split_rank = args.pipeline_model_parallel_split_rank
        if rank == -1:
            tensor_shapes.append(None)
        elif mpu.is_pipeline_stage_before_split(rank):
            # hidden_states ids_restore mask
            tensor_shapes.append((seq_length,args.micro_batch_size,args.hidden_size))
            tensor_shapes.append((args.micro_batch_size,num_patches))
            tensor_shapes.append((args.micro_batch_size,num_patches))
        elif rank==mpu.get_pipeline_model_parallel_world_size()-1: ## last_stage decoder
            tensor_shapes.append((num_patches,args.micro_batch_size,args.decoder_embed_dim))
            tensor_shapes.append((args.micro_batch_size,num_patches))
        else:## first stage or middle stage decoder 
            tensor_shapes.append((num_patches+1,args.micro_batch_size,args.decoder_embed_dim))
            tensor_shapes.append((args.micro_batch_size,num_patches))
